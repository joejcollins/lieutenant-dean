...
def get_top_two(value, data_store_group):
    ...
    filtered_list = [
        data_store for data_store in value if data_store_group in data_store["name"]
    ]
    return sorted(
...
class FilterModule(object):
    """Required FilterModule class that must have a method named filters.

    That will return a dictionary that maps filter names to callables implementing the
    filter.
